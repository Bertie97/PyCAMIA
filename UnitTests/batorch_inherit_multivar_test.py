...
#     def _get_range_(self, index):
#         start = self.inherited['tensor_starts'][index]
#         shape = self.inherited['shapes'][index]
#         return start, shape
    
#     def _get_mask_(self, index, value=1):
#         start, shape = self._get_range_(index)
#         shape_size = bt.tensor(shape[int(self.has_batch):]).prod()
#         value = bt.bttensor(value)
#         if not shape.has_batch:
#             value = value.flatten().init_special()
#             return bt.cat(bt.zeros({shape.n_batch}, start), bt.ones({shape.n_batch}, shape_size) * value, bt.zeros({shape.n_batch}, self.size(-1) - shape_size - start))
...
#         return bt.cat(bt.zeros(start), bt.ones(shape_size) * value, bt.zeros(self.size(-1) - shape_size - start))
    
#     def __getitem__(self, index):
#         if not isinstance(index, int): return super().__getitem__(index)
#         start, shape = self._get_range_(index)
#         return self[..., start:start+bt.tensor(shape[int(self.has_batch):]).prod()].view(shape)
    
#     def __setitem__(self, index, value):
#         if not isinstance(index, int): return super().__setitem__(index, value)
#         start, shape = self._get_range_(index)
#         return self * (1 - mask) + self._get_mask_(index, value)
    
#     @property
#     def main(self): return self[self.inherited['main_index']]
    
#     @main.setter
#     def main(self, value): self[self.inherited['main_index']] = value
        
#     @property
#     def main_grad(self): return self[self.inherited['main_grad_index']]
    
#     @main_grad.setter
#     def main_grad(self, value): self[self.inherited['main_grad_index']] = value
#
#     a, b = MV
#     print(a, b)
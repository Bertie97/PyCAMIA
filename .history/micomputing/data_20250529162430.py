    ...
    def data(self):
        ...
        for i, prop in enumerate(preprocess):
            ...
            if prop in self.dtypes:
                ...
                continue
        if self.property in self.dtypes: return data_tensor.type(self.property)
            
        if self.property in self.properties: return self.loaded_info.get(self.property, None)
        if self.subimage_rois:
            shape = tuple(s.stop - s.start for s in self.subimage_rois[0])
            data_tensor = bt.stack([bt.crop_as(self._data[roi], shape).squeeze() for roi in self.subimage_rois], {})
        else: data_tensor = self._data
        if self.property in self.dtypes: return data_tensor.type(self.property)
        elif self.property.startswith('range'):
            min_val, max_val = eval(self.property[len('range'):])

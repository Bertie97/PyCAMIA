...
def RigidProjectionError(D, mask = None):
    ...
    p_mod = (X / maxes).amplify(n_batch, []).flatten(2).mergedims(2, 0).with_channel_dim(None)
    p_obs = (Y / maxes).flatten(2).mergedims(2, 0).with_channel_dim(None) # ([n_batch x n_data], {n_dim})
    ...
    matT = T(r, s)
    hatY = (matT @ bt.cat((X / maxes).amplify(n_batch, []), bt.ones([n_batch], {1}), 1).flatten(2).with_channel_dim(None))[:, :-1].view_as(Y) * maxes

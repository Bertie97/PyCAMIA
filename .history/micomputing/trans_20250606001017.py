...
class FreeFormDeformation(Transformation):
    ...
    def __call_physical_space__(self, phyX):
        n_dim = self.n_dim
        avouch(self.source is not None and self.target is not None, "Cannot perform physical-space FFD transformation as FFD grid requires image coordinates to locate. ")
        taffine = Transformation.__get_affine__(self.target).type(phyX.dtype)
        saffine = Transformation.__get_affine__(self.source).type(phyX.dtype)
        if self.backward: prev_affine, post_affine = taffine, saffine
        else: prev_affine, post_affine = saffine, taffine

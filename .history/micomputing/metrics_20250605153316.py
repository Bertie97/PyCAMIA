...
class JointHistogram(bt.autograd.Function):
    ...
    def forward(ctx, I1: bt.Tensor, I2: bt.Tensor, nbin=100, mask=None):
        ...
        with bt.no_grad():
            ...
            collected = bt.Tensor(collected, batch_dim=True) / mask.sum(...) # ({n_batch}, n_bin, ..., n_bin)
    ...
    def backward(ctx, grad_output):
        ...
        grad_output = grad_output.as_subclass(bt.Tensor).init_special().with_i_batch_dim(True)
...
#             # collected = bt.Tensor(collected, batch_dim=True) # ({n_batch}, n_bin, ..., n_bin)
...
def MutualInformation(A, B, nbin=100, mask=None):
    ...
    if isinstance(A, bt.Tensor):
        ...
        if A.has_channel and not A.has_batch: i_channel = A.channel_dim; A.with_i_channel_dim(None).with_i_batch_dim(i_channel)
    ...
    if isinstance(B, bt.Tensor):
        ...
        if B.has_channel and not B.has_batch: B.with_i_channel_dim(None).with_i_batch_dim(i_channel)
    ...
    if i_batch is not None: MI.with_i_batch_dim(0).movedim_(0, i_batch)
    if i_channel is not None and i_channel == i_batch: MI.with_i_batch_dim(None).with_i_channel_dim(i_channel)
...
def NormalizedMutualInformation(A, B, nbin=100, mask=None):
    ...
    if i_channel is not None and i_channel == i_batch: NMI.with_i_batch_dim(None).with_i_channel_dim(i_channel)
...
def local_matrix(A: bt.Tensor, B: bt.Tensor, s=0, kernel="Gaussian", kernel_size=3):
    if isinstance(kernel, str):
        if kernel.lower() == "gaussian": kernel = bt.gaussian_kernel(n_dim = A.n_space_dim, kernel_size = kernel_size).unsqueeze(0, 0)
    ...
    def mean(a):
        ...
        if a.has_batch and a.has_channel: x = a
        elif not a.has_channel: x = a.unsqueeze([])
        elif not a.has_batch: x = a.unsqueeze({})
        else: x = a.unsqueeze({}, [])
    ...
    if s > 0:
        ...
        point_estim = bt.stack(bt.dot(GA, GA), bt.dot(GA, GB), bt.dot(GB, GB), dim=bt.FuncDim)
    else: point_estim = 0

    MA = mean(A)
    MB = mean(B)
    local_estim = bt.stack(mean(A * A) - MA ** 2, mean(A * B) - MA * MB, mean(B * B) - MB ** 2, dim=bt.FuncDim)
...
def CorrelationOfLocalEstimation(A, B, s=0, kernel="Gaussian", kernel_size=3):
    ...
    S11, S12, S22 = local_matrix(A, B, s=s, kernel=kernel, kernel_size=kernel_size)
...
def ITKMetric(A, B, spacing = 1, metric = "HD"):
    ...
    return resulting_measures.with_i_batch_dim(True)
...
def LocalNonOrthogonality(Disp_world_coord, mask = None, target_affine=None):
    ...
    JacOfPoints = gd.flatten(3).with_i_channel_dim(None).mergedims(3, 0) # ([n_batch x n_data(-dx)], n_dim, n_dim)
...
def RigidProjectionError(Disp_world_coord, mask = None, target_affine=None):
    ...
    p_mod = (X / maxes).flatten(3).mergedims(1, 3, 0).with_i_channel_dim(None)
    p_obs = (Y / maxes).flatten(3).mergedims(1, 3, 0).with_i_channel_dim(None) # ([n_batch x n_region x n_data], {n_dim})
    ...
    hatY = ((matT @ bt.cat(X.with_i_channel_dim(2) / maxes, bt.ones({n_batch}, n_region, {1}), {}).flatten(3).with_i_channel_dim(1))[:, :, :-1] * maxes).view({n_batch}, n_region, {n_dim}, *Disp_world_coord.shape[2:])
    return ((bt.Fnorm(Y - hatY).with_i_channel_dim(1) * mask).sum(...) / l).mean({})

...
with __info__:
    ...
    import numpy as np
    from pycamia import Path, Workflow, SPrint, Error, Warn
    from pycamia import no_print, touch, avouch, alias, get_environ_vars
    from pycamia import to_tuple, to_list, get_alphas, get_digits, get_snakename, arg_tuple
...
def affine2orient(affine: bt.Tensor):
    ...
    n_dim = affine.shape[-1] - 1
    if n_dim > 3: Warn("Affine matrix over 3D detected")
    orients = []
    for ia in range(n_batch):
        ...
        for d in range(n_dim):
            ...
            if i >= n_dim or j >= n_dim or i < 0 or j < 0:
                raise TypeError(f"Invalid affine matrix: {affine}")
            orient[j] = standard_axes[i][affine[ia, i, j] > 0]
            aff_abs[i] = 0
            aff_abs[:, j] = 0
...
def orient2affine(orient: str):
    n_dim = len(orient)
    if n_dim > 4: Warn("Orientation over 4D detected. ")
    rows = []
    for d in orient:
        for i, axis in enumerate(standard_axes):
            if d in axis:
                rows.append([-1, 1][axis.index(d)] * bt.one_hot(i, n_dim+1))
                break
    rows.append(bt.one_hot(n_dim, n_dim+1))
    return bt.stack(rows, [0])

def spacing2affine(spacing: tuple):
    n_dim = len(spacing)
    if n_dim > 4: Warn("Spacing over 4D detected. ")
    return bt.diag(bt.tensor(spacing + (1,), dtype=bt.float32).view([n_dim+1]))
...
class IMG:
    ...
    def IMGmethod(func):
        # A decorator that outputs the iamge for methods of IMG. 
        @functools.wraps(func)
        def wrapper(self, *args, **kwargs):
            try:
                return func(self, *args, **kwargs)
            except Exception as e:
                title = f"Error occurred in IMG({self.path}).{func.__name__}"
    ...
    def __init__(self, x: bt.Tensor, **kwargs):
        ...
        x = x.detach().cpu()
        dims = list(range(x.n_dim))
        s = min(x.seque_domain[0], x.space_domain[0])
        e = max(x.seque_domain[1], x.space_domain[1])
        rs = None if s == 0 else s-1
        re = s if e == 0 else e-1
        dims = dims[:s] + dims[re:rs:-1] + dims[e:]
        x = x.permute(*dims)
        self.unresolved_warning = False
    ...
    with run(sitk_supported_modalities), run.any_tag:
        def __init_sitk__(self, path, **kwargs):
            ...
            self.sid = ''
            if kwargs.pop('warn', True): self.image = sitk.ReadImage(path)
            else:
                with no_print: self.image = sitk.ReadImage(path)
            self.file_list = [path]
    ...
    with run("DCM"), run.use_tag:       
        def __init_dcm__(self, path, **kwargs):
            ...
            self.series_infos = {}
            self.basics = {}
            if kwargs.pop('warn', True): self.series_IDs = sitk.ImageSeriesReader.GetGDCMSeriesIDs(path)
            else:
                with no_print: self.series_IDs = sitk.ImageSeriesReader.GetGDCMSeriesIDs(path)
        ...
        def _handle_dcm_non_uniform_exception(self, file_list, series_name):
            ...
            for file_path in file_list:
                with dcm.dcmread(file_path, stop_before_pixels=True, force=True) as dcm_header:
                    ...
                    z = dcm_header[0x0020, 0x1041].value # Image Location
                    location_files.setdefault(z, [])
                    location_files[z].append(file_path)
            ...
            if len(new_critical_pos) == 0:
                # No info entry can seperate the series. Try to seperate by filenames.
                file_partitions = []
                for loc, loc_list in location_files.items():
                    if len(file_partitions) == 0:
                        for f in loc_list: file_partitions.append([f])
                        continue
                    located = []
                    for f in loc_list:
                        most_likely = (0., None)
                        for i, part in enumerate(file_partitions):
                            if i in located: continue
                            sim = len([1 for u, v in zip(f, part[0]) if u == v]) / len(f)
                            if sim > most_likely[0]: most_likely = (sim, i)
                        file_partitions[most_likely[1]].append(f)
                if len(file_partitions) != 1:
                    return [x for _, x in sorted([(''.join([chars[0] for chars in zip(*part) if len(set(chars)) == 1]), part) for part in file_partitions])]
        ...
        def _read_dcm(self, sid, file_list, info):
            ...
            if not isinstance(res[0][0], dict): # series with components
                for i, files in enumerate(res):
                    key = sid + f'.{i+1}.0.0'
                    sep = self._read_dcm(key, files, info)
                    if not sep: self.series_IDs.append(key)
                channels = []
                for i, (key, image) in enumerate(sorted([(key, self.images[key]) for key in self.series_IDs if key.startswith(sid) and key != sid])):
                    channels.append(image)
                    self.series_IDs.remove(key)
                    self.series_infos.pop(key)
                    self.file_lists.pop(key)
                    if key in self.images: self.images.pop(key)
                self.images[sid] = sitk.ComposeImageFilter().Execute(*channels)
                return False
    ...
    def to_numpy(self, sid=None):
        ...
        return image_data.transpose(*range(image_data.ndim)[::-1])
    ...
    def to_tensor(self, sid=None):
        ...
        if data_tensor.n_dim > len(self.shape): data_tensor = data_tensor.with_n_feature_dim(data_tensor.n_dim - len(self.shape))
    ...
    def from_array(self, array: bt.Tensor):
        ...
        dims = list(range(array.n_dim))
        s = min(array.seque_domain[0], array.space_domain[0])
        e = max(array.seque_domain[1], array.space_domain[1])
        rs = None if s == 0 else s-1
        re = s if e == 0 else e-1
        dims = dims[:s] + dims[re:rs:-1] + dims[e:]
    ...
    def path(self):
        if self.has_series:
            flist = self.file_lists[self.series_IDs[0]]
        else: flist = self.file_list
        if len(flist) == 0: raise RuntimeError(f"Got empty file list in image {self}. ")
        path = Path(flist[0])
        if len(flist) == 1: return path
        return path.dirname
    ...
    def reorient(self, orient='LPI'):
        ...
        dims = list(range(output_data.n_dim))
        s = min(output_data.seque_domain[0], output_data.space_domain[0])
        e = max(output_data.seque_domain[1], output_data.space_domain[1])
        rs = None if s == 0 else s-1
        re = s if e == 0 else e-1
        dims = dims[:s] + dims[re:rs:-1] + dims[e:]
        output_data = output_data.permute(*dims)
    ...
    def save(self, path: (str, callable), as_type=None, avoid_conflict=False):
        ...
        def get_path(sid, info):
            if callable(path): p = path(sid, info)
            elif isinstance(path, str): p = Path(path.format(sid=sid, **{''.join(k.split()):get_snakename(v) for k, v in info.items()}))
                ...
                for sid in ids:
                    ...
                    if p in used_path:
                        if avoid_conflict:
                            ...
                            p = p + f' ({i})' + ext
                        else: Warn(f"overwriting file {p} as a same path was given for multiple series. ")

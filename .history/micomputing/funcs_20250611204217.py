...
class registration:
    ...
    def align(self, trans_cls: Transformation, down_sample=6, max_iter=200, step_length=1e-3, loss='NMI', bend_coeff=1e-5, tol=1e-4, control_spacing=20):
        source = self.source
        target = self.target
        ...
        for i in range(max_iter):
            ...
            if len(prev_sim) > 5:
                prev_sim.pop(0)
                if abs(prev_sim[-1] - prev_sim[0]) / 5 / abs(prev_sim[-1]) < tol: break

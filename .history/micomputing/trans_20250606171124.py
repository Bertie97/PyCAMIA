...
class FreeFormDeformation(Transformation):
    ...
    def __init__(self, offsets, spacing=1, origin=0, preaffine=None, **kwargs):
        ...
        super().__init__(offsets, spacing=spacing, origin=origin, preaffine=preaffine, **kwargs)
    ...
    def random_init_params(cls, *shape, n_batch=1, spacing=None, std=1):
        ...
        ffd_gridsize = [math.ceil(l / s) + 1 for l, s in zip(shape, spacing)]
    ...
    def __call_physical_space__(self, phyX):
        n_dim = self.n_dim
        if self.preaffine is not None: prev_affine = self.preaffine
        elif self.backward:
        ...
        self.trans_record.append(f"- [In phy-FFD]: Affine Transformation with matrix:\n{prev_affine}")
        return self.__call_image_space__(imgX) - imgX - bt.channel_tensor(self.origin) + phyX

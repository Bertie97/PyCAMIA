...
class Transformation:
    ...
    def __call__(self, X, domain=None, use_implementation=None, to_space=None, **kwargs):
        ...
        if domain == core_domain:
            R = func(Y)
            self.trans_record.append(f"- {self.__name__} Transformation with param {self.params!r}")
            if not return_image:
            ...
            if self.__class__ in [Affine, Rigid, PolyAffine, LocallyAffine]:
                self.trans_record.append(f"- {self.__name__} Transformation with param {self.params[0][0]}")

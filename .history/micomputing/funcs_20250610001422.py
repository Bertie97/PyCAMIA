...
class registration:
    ...
    def align(self, trans_cls: Transformation, down_sample=6, max_iter=200, step_length=1e-3, loss='NMI', bend_coeff=1e-3, control_spacing=20):
        ...
        if trans_cls in [Affine, Rigid, PolyAffine, LocallyAffine]:
            ...
            two_stage = True
        if trans_cls == FreeFormDeformation:
            params = trans_cls.random_init_params(*target.space, n_batch=target.n_batch, std=1e1, **specific_params)
        else: params = trans_cls.random_init_params(*target.space, n_batch=target.n_batch, std=1e-3, **specific_params)
        # print(params[0].shape, target.shape, target_scalemat, control_spacing, specific_params)
        # print(target_aff)
        ...
        for i in range(max_iter):
            ...
            if trans_cls == FreeFormDeformation:
                print(repr(raw_trans.trans.offsets))

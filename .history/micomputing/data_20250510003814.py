...
def is_subkey(key_sub, key_cons):
    if not isinstance(key_cons, Key): return False
    for k, v in key_cons.items():
        if v == ... or isinstance(v, list) and len(v) == 1: continue
        if isinstance(v, (list, tuple)):
            cv = key_sub.get(k, None)
            if cv in v: ...
            elif cv == v: ...
            else: return False
...
class BatchObject:
    def __init__(self, obj=None, values=None, **kwargs):
        ...
        if values is None:
            if isinstance(obj, dict):
                self.names, self._values = zip(*obj.items())
        ...
        for value in self._values[0]:
            nb = getattr(value, '__len__', lambda:1)()
            if self.n_batch is not None: assert nb == self.n_batch, f"All values in BatchObject should have the same length, but got {nb} and {self.n_batch}. "
            else: self.n_batch = nb
    ...
    def __getitem__(self, i):
        ...
        return BatchObject(self._names, [v[i] for v in self._values])
    def items(self):
    ...
    def __str__(self):
        ...
        else: str_ = "BatchObject{" + ', '.join([f"{k} = {v}" for k, v in self.items()]) + "}"
...
class Dataset:
    ...
    def __call__(self, prot):
        ...
        if not isinstance(prot, type):
            ...
            if issubclass(prot, protocol): prot = prot("get_key", optional="get_batch")
            else: prot = prot()
    ...
    def sort_files(self):
        ...
        for d in self.directories:
            for f in d.iter_files():
                ...
                if not isinstance(key, Key): raise TypeError(f"The 'get_key' method in dataset protocal should return a Key object, but got {key}. ")
        ...
        for key in self.data:
            ...
            for bpk in self.batch_pattern.values():
                if bpk is ... or isinstance(bpk, list) and len(bpk) == 1: found_pattern[bpk] = True; continue
                assert isinstance(bpk, Key), "The batch pattern should be a list of Key (or placeholders ... or [i]) objects."
                pair_index = Key()
                pair_keys = {bpk: key}
                for n, p in bpk.items():
                    if p is ...: pair_index[n] = key[n]
                    elif isinstance(p, list) and len(p) == 1: pair_index[n + f'[{p[0]}]'] = key[n]
                for trybpk in self.batch_pattern.values():
                    if trybpk == bpk: continue
                    if trybpk is ... or isinstance(trybpk, list) and len(trybpk) == 1: continue
                    this_key = trybpk.copy()
                    for n, p in trybpk.items():
                        if p is ...: this_key[n] = pair_index[n]
                        elif isinstance(p, list) and len(p) == 1:
                            if n + f'[{p[0]}]' not in pair_index: continue
                            this_key[n] = pair_index[n + f'[{p[0]}]']
                    ...
                    for p in pair_keys.keys():
                        ...
                        self.batch_index[p][pair_index] = {k: v.to_property(k) if isinstance(v, Key) else ([u.to_property(k) if isinstance(u, Key) else u for u in v] if isinstance(v, list) else v) for k, v in pair_keys.items()}
    ...
    def batch(self, stage='training', n_batch=4, shuffle=True, drop_last=True, none_each_epoch=True, restart=False):
        ...
        while True:
            ...
            if self.use_subimages and self.batch_pointer[stage]['i_sub'] > 0:
                ...
                if batch_obj.n_subimage - i_sub <= remain_batch_size:
                    batch_list.append(batch_obj[i_sub:])
                    self.batch_pointer[stage]['i_sub'] = 0
                    remain_batch_size -= batch_obj.n_subimage - i_sub
                    continue
            ...
            for i, bp in enumerate(self.batch_pattern.values()):
                if bp in key_indices:
                    ...
                    elif isinstance(bp, list) and len(bp) == 1:
                        cands = [(k, v) for k, v in decided_key.items() if k.endswith(f"[{bp[0]}]")]
            ...
            for bp in self.batch_pattern:
                ...
                if key_cands._property_only is None: key_cands = key_cands.data
                chosen.append(key_cands)
            batch_obj = self.key2batch(self.data, *chosen)
            if hasattr(batch_obj, 'subimage_rois'):
                self.use_subimages = True
                n_subimage = batch_obj.n_subimage
        ...
        batch_values = []
        print(*[batch_item.values() for batch_item in batch_list])
        for values in zip(*[batch_item.values() for batch_item in batch_list]):
            if isinstance(values[0], list): cated = sum(values, [])
            elif isinstance(values[0], tuple): cated = sum(values, tuple())
            elif isinstance(values[0], bt.Tensor): cated = bt.cat(values, {})
            elif isinstance(values[0], bt.torch.Tensor): cated = bt.cat(values, 0)
            # else: cated = list(values)

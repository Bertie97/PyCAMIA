...
class registration:
    ...
    def align(self, trans_cls: Transformation, down_sample=6, max_iter=200, step_length=1e-3, loss='NMI', bend_coeff=1e-5, tol=1e-5, control_spacing=20):
        source = self.source
        target = self.target
        ...
        if trans_cls in [FreeFormDeformation, DenseDisplacementField]:
            params = trans_cls.random_init_params(*target.space, n_batch=target.n_batch, std=1e-2, **specific_params)
            param_scale = max(control_spacing) / 2
            if not all(s > 2 for s in params[0].space): return
        else:
        ...
        for i in range(max_iter):
            ...
            prev_sim.append(sim.item())
            n_sim = 5
            if len(prev_sim) > n_sim:
                prev_sim.pop(0)
                if abs(prev_sim[-1] - prev_sim[0]) / n_sim / abs(prev_sim[-1]) < tol: break
            if hasattr(raw_trans, 'displacements'):
                bend = bending(raw_trans.displacements)
                optimizer.maximize(sim - bend_coeff * bend)
                if verbose: print(f"Reg. Stage {self.stage} [{trans_cls.__name__}]: iteration = {i+1}, SIM = {sim.item():.4f}, bend = {bend.item():.2e}")
            else:
                optimizer.maximize(sim)
                if verbose: print(f"Reg. Stage {self.stage} [{trans_cls.__name__}]: iteration = {i+1}, SIM = {sim.item():.4f}")

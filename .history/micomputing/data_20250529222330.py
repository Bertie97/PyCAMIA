    ...
    def data(self):
        ...
        for i, prop in enumerate(preprocess):
            if prop in self.properties:
                if i < len(self.property) - 1:
                    raise TypeError(f"Cannot use property after {prop!r}: {self.property[i+1:]}. ")
            ...
            if isinstance(prop, str):
                if prop.startswith('flip'):
                    ...
                    if prop in self.dtypes:
                        data_tensor = data_tensor.type(self.property)
    ...
    def __call__(self, prot):
        ...
        if not isinstance(prot, type):
            ...
            if issubclass(prot, protocol):
                try: prot = prot("get_key", optional="get_batch", allow_more=True, allow_properties=False)
                ...
                for trybpk in self.batch_pattern.values():  # Test if the placeholder values can find the data. 
                    ...
                    while len(possible_keys) > 0:
                        ...
                        for n, v in k.items():
                            ...
                            if self.get_key(k): pair_keys[trybpk].append(k)

...
class registration:
    ...
    def align(self, trans_cls: Transformation, down_sample=6, max_iter=200, step_length=1e-3, loss='NMI', bend_coeff=1e-3, FFD_spacing=20):
        source = self.source
        target = self.target
        ...
        if trans_cls == FreeFormDeformation:
            ...
            pix_spacing = tuple(fsp / sp for fsp, sp in zip(FFD_spacing, affine2spacing(target_aff)))
        ...
        # print(params[0].shape, target.shape, target_scalemat, FFD_spacing, specific_params)
        # print(target_aff)

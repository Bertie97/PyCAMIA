...
class registration:
    ...
    def align(self, trans_cls: Transformation, down_sample=6, max_iter=200, step_length=1e-3, loss='NMI', bend_coeff=1e-3, FFD_spacing=20):
        ...
        if trans_cls == FreeFormDeformation:
            specific_params['FFD_affine'] = self.target_aff

        optimizer = bt.Optimization(bt.optim.Adam, params, lr=step_length)
        for i in range(max_iter):
            if trans_cls == FreeFormDeformation: print(target.shape, params, source_aff, target_aff, specific_params)
            raw_trans = trans_cls(*params, domain='physical-space', source=source_aff, target=target_aff, **specific_params)
            ...
            if hasattr(raw_trans, 'displacements'):
                optimizer.maximize(sim - bend_coeff * bending(raw_trans.displacements))
            else: optimizer.maximize(sim)

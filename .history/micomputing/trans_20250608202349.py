...
class FreeFormDeformation(Transformation):
    ...
    def __call_physical_space__(self, phyX):
        ...
        if self.origin is not None:
            ...
            print(prev_affine, self.origin)
            print(bt.matmul(prev_affine[..., :n_dim, :n_dim], bt.channel_tensor(self.origin)) + prev_affine[..., :n_dim, n_dim:])
            self.origin = (bt.matmul(prev_affine[..., :n_dim, :n_dim], bt.channel_tensor(self.origin)) + prev_affine[..., :n_dim, n_dim:]).view({-1}, [n_dim])
        else:
            phy_origin = None

    ...
    def test_tokenize_getargexp(self):
        ...
        for case in [
            dict(
                ...
                output = "(2, 3, this_to_be(extracted))"
            ),
            dict(
                inputs = dict(
                    line="z(sum(2, 3).x)))",
            ...
            dict(
                inputs = dict(
                    line="show(Tensor.inherit_from(obj, pivot, shape=...))",
                    func_name="show"
                ),
                output = "(Tensor.inherit_from(obj, pivot, shape=...))"
            ),
        ]:
            inputs = case["inputs"]
            output = case["output"]
            print("running:", ''.join(f"{k}={v!r}" for k, v in inputs.items()), "=>", f"{output!r}")
            self.assertEqual(run(**inputs), output)

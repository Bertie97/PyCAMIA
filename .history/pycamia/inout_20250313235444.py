...
class SPrint:
    ...
    def remove_line(self):
        ...
        if self._text.endswith('\n'):
            raw = self._text.rstrip('\n')
            end_rets = len(self._text) - len(raw)
        last_line_len = len(raw.rsplit('\n', 1)[-1])
        self._text = raw[:-last_line_len-1] + '\n' * end_rets
        return self
    
    def remove_last(self, n_char):
        self._text = self._text[:-n_char]
        return self

...
class protocol:
    def __new__(cls, *requires, optional=[], allow_more=False, allow_properties=True):
        """
        A base class for protocol.
        Args:
            requires: a list of method names that should be implemented.
            optional: a list of method names that are optional to be implemented.
            allow_more: whether to allow more unmentioned methods than required or optional.
            allow_properties: whether to allow uncallable properties.
        
        Examples::
            >>> def target_function(input_protocol):
            ...     if issubclass(input_protocol, protocol):
            ...         # define the requirements of the protocol
            ...         input_protocol = input_protocol('foo', 'bar')
            ...         # use the protocol
            ...         print(input_protocol.foo(1))
            ... 
            >>> class MyProtocol(protocol):
            ...     def foo(a): return a
            ...     def bar(a, b): return a + b
            ...
            >>> target_function(MyProtocol)
        """
        requires = list(requires)
        if isinstance(optional, tuple): optional = list(optional)
        if not isinstance(optional, list): optional = [optional]
        if allow_more: self.more = {}
        for name, value in cls.__dict__.items():
            if name.startswith('__') and name.endswith('__'): continue
            if not callable(value):
                if allow_properties: continue
                else: raise NameError(f"Protocol {cls.__name__} does not allow property member, but got {name!r}. ")
            if name in requires: requires.remove(name)
            elif name not in optional:
                if not allow_more: raise NameError(f"Protocol {cls.__name__} should not have {name!r} method. ")
                self.more[name] = value
            if callable(value):

                ...
                if issubclass(getattr(_type, '__origin__', _type), (tuple, list)) and isinstance(_type, types.GenericAlias):
                    ...
                    if isinstance(nargs, type):
                        ...
                        if isinstance(nargs, tuple):
                            assert len(nargs) == 2, "Only type and length can be set for list/tuple arguments. "
                            assert isinstance(nargs[0], type), f"The first item if list/tuple[*, 1] representation should be a type, instead of {type(args[0])}. "
                            _type = _type.__origin__[nargs[0]]
                            nargs = nargs[1]
                        else: _type = _type.__origin__

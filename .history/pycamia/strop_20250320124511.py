...
def refind(str_, regex):
    """
    Find by regex. 
    regex: requires 0, 1 (regex for token) or 3 (regexes for prefix, token, and suffix respectively) groups in regex.
    returns: a generator of tuples of (start_index, stop_index) for the token.
    """
    for m in re.finditer(regex, str_):
        n_group = len(m.groups())
        if n_group == 0: igroup = 0
        elif n_group == 1: igroup = 1
        elif n_group == 3: igroup = 2
        else: raise TypeError(f"Function 'refind' requires 0, 1 (regex for token) or 3 (regexes for prefix, token, and suffix respectively) groups in regex, not {n_group}. ")
        s, e = m.span(igroup)
        yield (s, e)

def resplit(str_, regsep):
    """
    Split by regex. 
    regsep: requires 0, 1 (regex for split token) or 3 (regexes for prefix, split token, and suffix respectively) groups in regex.
    """
    fregs = []
    p = 0
    for m in re.finditer(regsep, str_):
        n_group = len(m.groups())
        if n_group == 0: igroup = 0
        elif n_group == 1: igroup = 1
        elif n_group == 3: igroup = 2
        else: raise TypeError(f"Function 'resplit' requires 0, 1 (regex for split token) or 3 (regexes for prefix, split token, and suffix respectively) groups in regex, not {n_group}. ")
        s, e = m.span(igroup)
        fregs.append(str_[p:s])
        p = e
    if p < len(str_): fregs.append(str_[p:])
    return fregs

def rereplace(str_, regex, replacement):
    """
    Replace by regex. 
    regex: requires 0, 1 (regex for replace token) or 3 (regexes for prefix, replace token, and suffix respectively) groups in regex.
    """
    if isinstance(replacement, str): replacement = lambda _, r=replacement: r
    fregs = []
    p = 0
    for m in re.finditer(regex, str_):
        n_group = len(m.groups())
        ...
        else: raise TypeError(f"Function 'rereplace' requires 0, 1 (regex for replace token) or 3 (regexes for prefix, replace token, and suffix respectively) groups in regex, not {n_group}. ")
        s, e = m.span(igroup)
        fregs.append(str_[p:s])
        fregs.append(replacement(str_[s:e]))
        p = e
    if p < len(str_): fregs.append(str_[p:])
    return ''.join(fregs)
...
def format(format_str, *args, **kwargs):
    vars = [0, False]
    def format_replace(regmatch, vars=vars, args=args, kwargs=kwargs):
        i, manual = vars
        name, fmt = regmatch.groups()
        lstrip, rstrip = False, False
        new_fmt = re.sub(r"^-\.", ".", fmt)
        if new_fmt != fmt: lstrip = True; fmt = new_fmt
        new_fmt = re.sub(r"\.(\d+)-(\w)", lambda m: f".{m.group(1)}{m.group(2)}", fmt)
        if new_fmt != fmt: rstrip = True; fmt = new_fmt
        if name == '':
            if manual:
                raise ValueError("cannot switch from manual field specification to automatic field numbering")
            name = str(i)
            vars[0] += 1 # i += 1
        else: vars[1] = True # manual = True
        if re.match(r'\d+', name): value = args[eval(name)]
        else: value = kwargs[name]
        text = f"{{val:{fmt}}}".format(val=value)
        if lstrip: text = text.lstrip('0')
        if rstrip: text = text.rstrip('0')
        return text
    return re.sub(r"{(\w*):([^{}]*)}", format_replace, format_str)

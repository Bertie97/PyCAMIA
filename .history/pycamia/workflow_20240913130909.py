...
class Switch:
    ...
    Example #1::
        ...
        >>> case = 2
        >>> switch = Switch(case)
        >>> with switch(1) as this:
        ... 	assert this
        ... 	print("hi")
        ...
        >>> with switch(2) as this:
        ... 	assert this
        ... 	print('bye')
        ...
        bye
    Example #2 (NOTE: switch[=Switch()] here is in lower cases. )::
	    ...
	    ...     print('bye')
    """
    def __init__(self, variable=True):
        self.value = variable
        self.values = []
        self.matched = False
    def __call__(self, value):
        ...
        if value == self.value:
            self.matched = True
        return Switch(value == self.value)
    def __enter__(self): return self
    def __exit__(self, error_type, error_msg, traceback):
        if error_type is None:
            if self.values and not self.matched:
                raise Error("Switch")(f"Unrecognized case {self.value}, the available options are {self.values}. ")

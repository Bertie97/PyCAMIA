...
def do_package(name, version=None, pack_type="zipwheel", upload=True):
    ...
    if missing_require: Warn(f"package {name} imported {missing_require} but didnot declare requirement. ")
...
def pack(*package_names):
    ...
    if 'all' in package_names:
        if len(package_names) > 1:
            Warn("Indicator 'all' was found as long as ordinary package names. ")
        ...
        for pn in package_names:
            ...
            else: Warn(f"Package '{pn}' not found. ")

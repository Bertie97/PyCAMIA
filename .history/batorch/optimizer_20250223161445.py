...
class Optimization:
    ...
    def minimize(self, loss, **kwargs):
        ...
        Args:
            ...
            update_lr (bool): Whether update the learning rate. Defaults to True. 
        ...
        update_lr = kwargs.pop('update_lr', True)
        optimizer = self.optimizer(self._index)
        optimizer.zero_grad()
        loss.backward(**kwargs)
        optimizer.step()
        if update_lr: self._index += 1
...
def exp_lr(initial, terminate, n_iter):
    order = bt.arange(n_iter).float() / n_iter
    factor = bt.tensor(float(terminate) / float(initial)) ** order
    return lambda i: initial * factor[max(i, 1)-1].item()

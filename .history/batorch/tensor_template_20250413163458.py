...
@alias('tr')
def trace(input: 'Tensor', dim:linalg_dim[2]=None):
...
def square(input: 'Tensor', *, out=None): ...
...
def val_range(self, dim: exist_dim=None):
    ...
    """
    return stack(self.min(dim), self.max(dim), FuncDim) # suppress: special_from
...
def norm(input: 'Tensor', p='fro', dim: del_dim[...]=None, keepdim=False, dtype=None): ...
...
class Tensor(torch.Tensor):
    ...
    def view(self, *size):
        ...
        with torch._C.DisableTorchFunction():
            return Tensor.inherit_from(torch_super(self, 'view')(size), self, shape=size)
    ...
    def down_scale(self: 'Tensor', factor=1):
        ...
        return self[(slice(None),) * self.space_start + tuple(slice(None, None, f) for f in factor)] # suppress: special_from
...
def tensor(data, *, dtype=None, device=_device.main_device, requires_grad=False, pin_memory=False):
    if isinstance(data, torch.Tensor):
        if isinstance(data, bt.Tensor): return data.clone()
        else: return data
    with torch._C.DisableTorchFunction():

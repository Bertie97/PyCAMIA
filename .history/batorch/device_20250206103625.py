...
with __info__:
    ...
    from pycamia import alias, ByteSize, Warn
        ...
        for gpu_family in GPU_priority:
            if gpu_family == 'cuda' and is_available['cuda']:
                ...
                if len(candidates) > 0: most_available_gpus, most_available_gpu_memory = zip(*candidates)
                    ...
                    Warn(f"no gpu device is available (with memories {available_gpus_memory}), reset environment variable CUDA_RUN_MEMORY to change the memory required...")
                ...
                if self.required_memory and sum(most_available_gpu_memory) < self.required_memory and self.verbose:
                    Warn(f"all remaining memory of gpu devices is not enough (on: {most_available_gpus})...")
                    ...
                    Warn(f"not enough gpus available (available: {most_available_gpus})...")
                    print(f"Total free memory: {sum(most_available_gpu_memory) / GB:.5} GB. ")
                    if not self.always_proceed:
                        tag = input("Do you want to proceed? [yes/no/y/n]:")
            ...
            if self.verbose: Warn("cannot find any gpu, using cpu instead. ")

...
with __info__:
    from pycamia import alias, avouch, to_tuple, to_list, restore_type_wrapper, prod, Version, Path
...
def bending(disp, mask=None):
    ...
    return torch.divide((((Hes ** 2).sum([1, 2]) + eps).sqrt().mean(1) * torch.crop_as(mask, size, n_keepdim=1)).flatten(1).sum(1), mask.flatten(1).sum(1), 0.0).as_subclass(bt.Tensor).with_batchdim(0)
...
class local_prior:
    ...
    def local_rigidity(self, X, Y):
        ...
        for t in range(A.n_batch):
            ...
            if bt.__torchversion__ >= Version('1.10'): L, P = bt.linalg.eig(A[t]); l = L.real
            else:

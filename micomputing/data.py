...
class DataObject:
    ...
    def __init__(self, obj, loader=None, *, rois=None, prop=None):
        ...
        if isinstance(obj, str):
            ...
            if prop is None: self.property = obj.property
            elif obj.property is None: self.property = prop
            elif isinstance(obj.property, list): self.property = obj.property + [prop]
            else: self.property = [obj.property, prop]
    ...
    properties = ['shape', 'affine', 'header', 'orient']
    ...
    @property
    def long(self): return DataObject(self, prop='long')
    
    @property
    def float(self): return DataObject(self, prop='float')
    
    def range(self, min_val, max_val): return DataObject(self, prop=f'range({min_val}, {max_val})')
    def flip(self, axis): return DataObject(self, prop=f'flip({axis})')
    def respacing(self, *spacing): return DataObject(self, prop=f'respacing{spacing}')
    def crop(self, *shape): return DataObject(self, prop=f'crop{shape}')
    def select_channel(self, i_channel): return DataObject(self, prop=f'select_channel({i_channel})')
    ...
    def data(self):
        ...
        preprocess = self.property
        if preprocess is None: return self._data
        if not isinstance(preprocess, list): preprocess = [preprocess]
        if self.subimage_rois: preprocess = [self.subimage_rois] + preprocess
        
        info = self.loaded_info.copy()
        data_tensor = self._data
        for i, prop in enumerate(preprocess):
            if prop in self.properties:
                if i < len(preprocess) - 1:
                    raise TypeError(f"Cannot use property after {prop!r}: {preprocess[i+1:]}. ")
                return info.get(prop, None)
            
            if isinstance(prop, list): # ROIs
                shape = tuple(s.stop - s.start for s in prop[0])
                data_tensor = bt.stack([bt.crop_as(data_tensor[roi], shape).squeeze() for roi in prop], {})
                info['shape'] = shape
                continue
            if isinstance(prop, str):
                if prop.startswith('flip'):
                    axis = eval(prop[len('flip'):])
                    if isinstance(axis, str):
                        assert axis[0] in info['orient'] or axis[1] in info['orient'], f"Cannot find axis {axis!r} in {info['orient']}."
                        axis = info['orient'].index(axis[0]) if axis[0] in info['orient'] else info['orient'].index(axis[1])
                    prop = Reflect(axis, image_size=info['shape'])
                elif prop.startswith('respacing'):
                    voxel_size = eval(prop[len('respacing'):])
                    prop = Resample(*[tsp / ssp for ssp, tsp in zip(mc.affine2spacing(info['affine']), voxel_size)])
                elif prop.startswith('select_channel'):
                    i_channel = eval(prop[len('select_channel'):])
                    if data_tensor.has_channel: data_tensor = data_tensor.pick(i_channel, [])
                    continue
                elif prop.startswith('crop'):
                    size = eval(prop[len('crop'):])
                    data_tensor = bt.crop_as(data_tensor, size)
                    info['shape'] = size
                    continue
                else:
                    if prop in self.dtypes:
                        data_tensor = data_tensor.type(prop)
                    elif prop.startswith('range'):
                        min_val, max_val = eval(prop[len('range'):])
                        data_tensor = ((data_tensor - min_val) / (max_val - min_val)).clamp(0., 1.)
                    if prop == 'zscore':
                    ...
                    continue
            if isinstance(prop, str): prop = eval(prop)
            assert isinstance(prop, Transformation), f"Invalid preprocessor {prop}: should be a string (long|float|zscore|normalized|range(*,*)|flip(*)|respacing(**)|crop(**)) or a Transformation."
            affine = prop.affine(len(info['shape']))
            assert affine is not None, "Only Transformations with affine matrix can be used in DataObject. "
            data_tensor = prop(data_tensor, domain='non-spatial')
            info['shape'] = data_tensor.space
            info['affine'] = info['affine'] @ affine
            info['orient'] = mc.affine2orient(info['affine'])
        return data_tensor
...
class Dataset:
    ...
    def __call__(self, prot):
        ...
        if not isinstance(prot, type):
            ...
            properties = {}
            if isinstance(prot, type):
                for name, value in prot.__dict__.items():
                    if name.startswith('__') and name.endswith('__'): continue
                    if not callable(value): properties[name] = value
            if issubclass(prot, protocol):
                try:
                    prot = prot("get_key", optional="get_batch", allow_more=True, allow_properties=False)
                    for k, v in prot.more.items(): setattr(self, k, v)
                ...
                for trybpk in self.batch_pattern.values():  # Test if the placeholder values can find the data. 
                    ...
                    while len(possible_keys) > 0:
                        ...
                        for n, v in k.items():
                            ...
                            if touch(lambda: self.get_key(**k, all_candidates=True)): pair_keys[trybpk].append(k)
    ...
    def split_datasets(self, *, byprop = None, training = None, validation = None, testing = None, shuffle = True):
        ...
        if byprop is not None:
            ...
            for sid in main_keys:
                subset_name = touch(lambda: self.get_key(**{self.main_key_name:sid}, all_candidates=True))
                assert subset_name is not None, f"Internal error: invalid sid {sid!r}"
                if isinstance(subset_name, (tuple, list)): subset_name = subset_name[0]

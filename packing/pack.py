...
with __info__:
    ...
    from pycamia import touch, Error, enclosed_object, to_list, is_valid_command
    from import_all import import_all
    from check_import import check_import

usr, psw, *_ = open(os.path.join(os.pardir, os.pardir, "pypi.keychain")).read().split()
key_chain = {"usr": usr, "psw": psw}
# key_chain = {"usr": "None", "psw": "None"} # One can use plain text here.
home_path = os.pardir
...
def pack(*package_names):
    ...
    if b"403 Forbidden" in p.before: print("Upload Failed..."); return
    if b"400 Bad Request" in p.before: print("Upload Failed (A copy of given version found on the server, please correct the version manually)..."); return
    else:
        names = []
